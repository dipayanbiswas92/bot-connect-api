...
server.on('connection', ws => {
  ...
  messages[newMessageId] = {
    id: newMessageId,
    type: 'notification',
    text: `${users[connectedUserId].name} joined the room`,
    time: Date.now(),
    userId: connectedUserId,
    userName: users[connectedUserId].name
  }
    ...
    switch (action.type) {
      ...
      case NEW_MESSAGE: {
        ...
        messages[newMessageId] = {
          ...
          userName: users[action.payload.userId].name
        }
  ...
  ws.on('close', () => {
    ...
    messages[newMessageId] = {
      id: newMessageId,
      type: 'notification',
      text: `${disconnectedUser.name} left the room`,
      time: Date.now(),
